meovnSearch.service('firebaseService', ["$firebaseArray", "$filter", function ($firebaseArray, $filter) { 
	...
	var getOrderHistory = function(mobile){
		return ref.child('orders').orderByChild('buyer_mobile').equalTo(mobile).once('value', function(snapshot){
			return snapshot;
		 });
	}
